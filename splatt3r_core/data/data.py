...
class DUST3RSplattingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for c_view in context_views:
            ...
            # Upstream had `assert view['valid_mask'].any()` here. Removed:
            # it is the same failure mode already hit (and fixed) in
            # splatt3r_core/main.py's calculate_loss -- an exception raised
            # from ONE DDP rank's data while the other rank proceeds into
            # backward()'s gradient all-reduce hangs the run for the full
            # NCCL timeout (30 min) instead of failing fast. It is also
            # reachable in practice here, not hypothetical: euroc/eth3d
            # train against self-predicted pseudo-depth, and a frame whose
            # depth prediction is entirely below the confidence threshold
            # yields an all-zero depthmap and hence an empty valid_mask.
            # Warn instead -- keeping the sample is harmless on the
            # training path: nothing in training consumes view['valid_mask']
            # or view['pts3d'] (only utils/export.py's visualization export
            # reads them), the loss mask is recomputed from the depthmaps
            # by loss_mask.calculate_loss_mask (NOT this valid_mask -- the
            # two are unrelated), and mast3r_criterion -- the one loss that
            # would read ground-truth pts3d -- is not enabled
            # (config.loss.mast3r_loss_weight is None). Every rank must
            # still take the same control-flow path regardless of its own
            # batch's content.
            if not view['valid_mask'].any():
                print(
                    f"[warn] empty valid_mask for sequence={sequence} view={c_view} "
                    f"-- keeping sample, it will contribute ~zero loss",
                    flush=True,
                )
...
class DUST3RSplattingTestDataset(torch.utils.data.Dataset):
    ...
    def get_view(self, sequence, c_view):
        ...
        view['pts3d'] = pts3d
        view['valid_mask'] = valid_mask & np.isfinite(pts3d).all(axis=-1)
        # Same reasoning as the training-dataset path above: an all-zero
        # pseudo-depth frame (euroc/eth3d) is a real possibility, and
        # asserting here would abort a whole evaluation run over one bad
        # frame. No DDP-hang risk on this single-process path, but the
        # behaviour should still match.
        if not view['valid_mask'].any():
            print(
                f"[warn] empty valid_mask for sequence={sequence} view={c_view} "
                f"-- keeping sample",
